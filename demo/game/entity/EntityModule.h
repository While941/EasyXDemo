...
public:
	...
	void OnMove(EntityId entityId, const Vector2D<float>& target,double deltaTime);
	Player& GetPlayer(const RoleId& roleId) const;

	std::unordered_map<EntityId, std::unique_ptr<Player>>& GetPlayerMap() { return m_role_list; }

	static int entity_count;
private:
	std::unordered_map<EntityId, std::unique_ptr<Player>> m_role_list;
	std::unordered_map<EntityId, std::unique_ptr<Monster>> m_monster_list;
