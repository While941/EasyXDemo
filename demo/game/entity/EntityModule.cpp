...
{
	EntityId id = static_cast<EntityId>(m_entity_list.size() + 1);
	m_monster_list[id] = std::make_unique<Monster>(1);
	m_entity_list[id] = (Entity*)m_monster_list[id].get();
}
...
void EntityModule::OnUpdate(double detalTime)
{
	...
	for (auto& it : m_monster_list)
	{
		it.second.get()->OnUpdate(detalTime);
	}
}

void EntityModule::OnMove(EntityId entityId, const Vector2D<float>& target,double deltaTime)
{
