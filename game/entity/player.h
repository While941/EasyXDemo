...
public:
	...
	virtual void OnMove(const Vector2D<int>& target,double deltaTime );
	...
	void handleMessage1(const std::shared_ptr<MoveMessage>& message) {
		// 处理消息...
		const Vector2D<int>& target = message->position;
		//OnMove(target);
	}

	EntityId GetEntityId() const { return m_entity_id; }
public:
	Vector2D<int> GetPlayerPoint() const { return player_point; }
...
private:
	...
	Vector2D<int> player_point = { 500,500 };
